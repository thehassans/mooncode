...
router.post("/ai/test", auth, allowRoles("admin", "user"), async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      const descModel =
        body.geminiDescModel || cur.geminiDescModel || "gemini-1.5-pro";
...
router.post("/seo", auth, allowRoles("admin", "user"), async (req, res) => {
  try {
    ...
    if (typeof siteDescription === "string")
      value.siteDescription = siteDescription.trim();
    ...
    if (typeof googleAnalytics === "string")
      value.googleAnalytics = googleAnalytics.trim();
    if (typeof facebookPixel === "string")
      value.facebookPixel = facebookPixel.trim();
    if (typeof tiktokPixel === "string") value.tiktokPixel = tiktokPixel.trim();
    if (typeof structuredData === "boolean")
