  ...
  async (req, res) => {
    ...
    }
    {
      ...
      if (!isAllowedCountry) {
        return res.status(400).json({
          message:
            "Phone number must be from UAE (+971), Oman (+968), KSA (+966), Bahrain (+973), Pakistan (+92), Kuwait (+965), Qatar (+974), or India (+91)",
        });
      }
    }
    const exists = await User.findOne({ email });
    ...
    (async () => {
      try {
        ...
        try {
          console.error(
            "[agents] failed to send welcome WA",
            err?.message || err
          );
        } catch {}
      }
  ...
  async (req, res) => {
    try {
      ...
      const users = await User.find(
        cond,
        "firstName lastName email phone country city role driverProfile createdAt"
      )
    ...
    try {
      ...
      if (phone !== undefined) {
        ...
        if (!isAllowedCountry)
          return res.status(400).json({
            message:
              "Phone must start with +971 (UAE), +968 (Oman), +966 (KSA), +973 (Bahrain), +92 (Pakistan), +965 (Kuwait), +974 (Qatar) or +91 (India)",
          });
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      return res
        .status(500)
        .json({ message: err?.message || "Failed to load investor plans" });
    }
  }
);
  ...
  async (req, res) => {
    try {
      ...
      const clean = [1, 2, 3].map((idx) => {
        ...
        const name = String(found.name || `Products Package ${idx}`).slice(
          0,
          80
        );
        ...
        const profitPercentage = Math.max(
          0,
          Math.min(100, Number(found.profitPercentage || 0))
        );
        const files = Array.isArray(req.files) ? req.files : [];
        const fx = files.find(
          (f) => String(f.fieldname).toLowerCase() === `image${idx}`
        );
      ...
      try {
        ...
        for (const inv of investors) {
          io.to(`user:${String(inv._id)}`).emit("investor-plans.updated", {
            owner: ownerId,
          });
        }
      } catch {}
      ...
      return res
        .status(500)
        .json({ message: err?.message || "Failed to update investor plans" });
    }
  }
);
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      const list = await InvestorRequest.find(base)
        .sort({ createdAt: -1 })
      ...
      return res
        .status(500)
        .json({ message: err?.message || "Failed to load investor requests" });
    }
  }
);

router.post(
  "/investor-requests/:id/accept",
  auth,
  allowRoles("admin", "user"),
  async (req, res) => {
    try {
      ...
      if (inv) {
        ...
        try {
          const io = getIO();
          io.to(`user:${String(inv._id)}`).emit("investor.updated", {
            ok: true,
          });
        } catch {}
      }
      try {
        const io = getIO();
        io.to(`workspace:${String(rq.owner)}`).emit(
          "investor.request.accepted",
          { id: String(rq._id) }
        );
      } catch {}
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      try {
        const io = getIO();
        io.to(`user:${String(rq.investor)}`).emit("investor.updated", {
          ok: true,
        });
      } catch {}
      try {
  ...
  async (req, res) => {
    ...
    (async () => {
      try {
        ...
        try {
          console.error(
            "[managers] failed to send welcome WA",
            err?.message || err
          );
        } catch {}
      }
...
// Referral summary for owner (admin,user)
router.get(
  "/referrals/summary",
  auth,
  allowRoles("admin", "user"),
  async (req, res) => {
    try {
      const ownerId =
        req.user.role === "admin" ? req.query.ownerId || null : req.user.id;
      const base = { role: "investor" };
      if (ownerId) base.createdBy = ownerId;
      const investors = await User.find(base, "-password")
        .select(
          "firstName lastName email phone createdAt investorProfile referredBy referredByCode referralCode refCode inviteCode"
        )
        .populate("referredBy", "firstName lastName email")
        .lean();
      if (!investors.length) return res.json({ referrals: [] });

      const ids = investors.map((i) => i._id);
      const orders = await Order.aggregate([
        { $match: { "investorProfit.investor": { $in: ids } } },
        { $sort: { createdAt: -1 } },
        {
          $group: {
            _id: "$investorProfit.investor",
            orderCount: { $sum: 1 },
            totalProfit: {
              $sum: { $ifNull: ["$investorProfit.profitAmount", 0] },
            },
        ...
        },
        {
          $project: {
            ...
            totalProfit: 1,
            totalOrderValue: 1,
            latestOrders: { $slice: ["$latestOrders", 5] },
          },
        },
      ]);
      ...
      const out = investors.map((inv) => {
        ...
        return {
          ...
          referredByCode: inv.referredByCode || "",
          referralCode: inv.referralCode || inv.refCode || inv.inviteCode || "",
          orderStats: {
      ...
      return res.json({ referrals: out });
    } catch (err) {
      return res
        .status(500)
        .json({ message: err?.message || "Failed to load referrals" });
    }
  }
);
...
router.post(
  ...
  async (req, res) => {
    ...
    if (!profitAmount || profitAmount <= 0)
      return res.status(400).json({ message: "Profit amount is required" });
    ...
    const profitPct = Math.max(
      0,
      Math.min(100, Number(profitPercentage || 15))
    ); // Default 15%, max 100%
    ...
    try {
      const io = getIO();
      io.to(`workspace:${createdBy}`).emit("investor.created", {
        id: String(investor._id),
      });
      io.to(`user:${String(investor._id)}`).emit("investor.updated", {
        id: String(investor._id),
      });
    } catch {}
    ...
    (async () => {
      try {
        ...
        try {
          console.error(
            "[investors] failed to send welcome WA",
            err?.message || err
          );
        } catch {}
      }
...
router.post(
  ...
  async (req, res) => {
    ...
    if (
      req.user.role !== "admin" &&
      String(investor.createdBy) !== String(req.user.id)
    ) {
      return res.status(403).json({ message: "Not allowed" });
    }
    ...
    if (investor.investorProfile.status !== "completed") {
      if (investmentAmount !== undefined) {
        ...
        investor.investorProfile.totalReturn =
          invAmount + (investor.investorProfile.earnedProfit || 0);
      }

      if (profitAmount !== undefined) {
        investor.investorProfile.profitAmount = Math.max(
          0,
      ...
      if (profitPercentage !== undefined) {
        investor.investorProfile.profitPercentage = Math.max(
          0,
          Math.min(100, Number(profitPercentage || 15))
        );
      }
    ...
    try {
      ...
      io.to(`workspace:${ownerId}`).emit("investor.updated", {
        id: String(investor._id),
      });
      io.to(`user:${String(investor._id)}`).emit("investor.updated", {
        id: String(investor._id),
...
router.delete(
  ...
  async (req, res) => {
    ...
    if (
      req.user.role !== "admin" &&
      String(inv.createdBy) !== String(req.user.id)
    ) {
      return res.status(403).json({ message: "Not allowed" });
    }
    await User.deleteOne({ _id: id });
  ...
  async (req, res) => {
    ...
    if (phone) {
      const allowedCodes = [
        ...
        "+974",
        "+91",
      ]; // UAE, Oman, KSA, Bahrain, Kuwait, Qatar, India
      const phoneClean = String(phone).replace(/\s/g, "");
      const isAllowedCountry = allowedCodes.some((code) =>
        phoneClean.startsWith(code)
      );
      ...
      if (!isAllowedCountry) {
        return res.status(400).json({
          message:
            "Phone number must be from UAE (+971), Oman (+968), KSA (+966), Bahrain (+973), Kuwait (+965), Qatar (+974) or India (+91)",
        });
      }
    }
    ...
    (async () => {
      try {
        ...
        try {
          console.error(
            "[drivers] failed to send welcome WA",
            err?.message || err
          );
        } catch {}
      }
    ...
    try {
      ...
      if (phone !== undefined) {
        ...
        if (!isAllowedCountry)
          return res.status(400).json({
            message:
              "Phone must start with +971 (UAE), +968 (Oman), +966 (KSA), +973 (Bahrain), +965 (Kuwait), +974 (Qatar) or +91 (India)",
          });
