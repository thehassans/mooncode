...
// Driver submits cancelled/returned order back to company
router.post('/:id/submit-return', auth, allowRoles('driver'), async (req, res) => {
  try{
    ...
    if (String(order.deliveryBoy) !== String(req.user.id)){
      return res.status(403).json({ message: 'Not authorized - order not assigned to you' })
    }
    
    // Check if order is cancelled or returned
    const status = String(order.shipmentStatus || '').toLowerCase()
    if (!['cancelled', 'returned'].includes(status)){
      return res.status(400).json({ message: 'Order must be cancelled or returned to submit' })
    }
    
    // Mark as submitted by driver
    order.driverSubmittedReturn = true
    order.driverSubmittedReturnAt = new Date()
    await order.save()
    
    // Notify owner/manager
    try{
      const io = getIO()
      io.to(`user:${String(order.createdBy)}`).emit('order.return-submitted', { orderId: String(order._id) })
      // Also notify manager if exists
      const owner = await User.findById(order.createdBy).select('role createdBy').lean()
      if (owner?.role === 'agent' && owner?.createdBy){
        io.to(`user:${String(owner.createdBy)}`).emit('order.return-submitted', { orderId: String(order._id) })
      }
    }catch{}
    
    return res.json({ message: 'Return submitted successfully', order })
  }catch(err){
    console.error('Submit return error:', err)
    return res.status(500).json({ message: 'Failed to submit return' })
  }
})

// User/Manager verifies driver submitted return
router.post('/:id/verify-return', auth, allowRoles('admin','user','manager'), async (req, res) => {
  try{
    ...
    // Check permissions
    if (req.user.role === 'user' && String(order.createdBy) !== String(req.user.id)){
      return res.status(403).json({ message: 'Not authorized' })
    }
    if (req.user.role === 'manager'){
      const agent = await User.findById(order.createdBy).select('createdBy').lean()
      if (String(agent?.createdBy) !== String(req.user.id)){
        return res.status(403).json({ message: 'Not authorized' })
      }
    }
    
    // Check if driver submitted return
    if (!order.driverSubmittedReturn){
      return res.status(400).json({ message: 'Driver has not submitted this return yet' })
    }
    ...
    try{
      const io = getIO()
      io.to(`user:${String(order.deliveryBoy)}`).emit('order.return-verified', { orderId: String(order._id) })
    }catch{}
    
    return res.json({ message: 'Return verified successfully', order })
  }catch(err){
    console.error('Verify return error:', err)
    return res.status(500).json({ message: 'Failed to verify return' })
  }
})
