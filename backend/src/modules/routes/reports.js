...
router.get("/user-metrics", auth, allowRoles("user"), async (req, res) => {
  try {
    ...
    const orderStats = await Order.aggregate([
      { $match: { createdBy: { $in: creatorIds }, ...dateMatch } },
      {
        $group: {
          ...
          pickedUpOrders: {
            $sum: { $cond: [{ $eq: ["$shipmentStatus", "picked_up"] }, 1, 0] },
          },
          deliveredOrders: {
            $sum: { $cond: [{ $eq: ["$shipmentStatus", "delivered"] }, 1, 0] },
          },
          cancelledOrders: {
            $sum: { $cond: [{ $eq: ["$shipmentStatus", "cancelled"] }, 1, 0] },
          },
    ...
    const countryMetrics = await Order.aggregate([
      ...
      },
      {
        $group: {
          ...
          assignedOrders: {
            $sum: { $cond: [{ $eq: ["$shipmentStatus", "assigned"] }, 1, 0] },
          },
          pickedUpOrders: {
            $sum: { $cond: [{ $eq: ["$shipmentStatus", "picked_up"] }, 1, 0] },
          },
          inTransitOrders: {
            $sum: { $cond: [{ $eq: ["$shipmentStatus", "in_transit"] }, 1, 0] },
          },
        ...
        { $unwind: "$_items" },
        {
          $group: {
            ...
            qty: {
              $sum: {
                $cond: [
                  { $lt: [{ $ifNull: ["$_items.quantity", 1] }, 1] },
                  1,
                  { $ifNull: ["$_items.quantity", 1] },
                ],
              },
            },
          },
    ...
    countryMetrics.forEach((cm) => {
      ...
      countries[key].orders =
        (countries[key].orders || 0) + (cm.totalOrders || 0);
      countries[key].pickedUp =
        (countries[key].pickedUp || 0) + (cm.pickedUpOrders || 0);
      countries[key].delivered =
        (countries[key].delivered || 0) + (cm.deliveredOrders || 0);
      countries[key].transit =
        (countries[key].transit || 0) + (cm.inTransitOrders || 0);
      // additional status counts (use openOrders for "pending" tile semantics)
      countries[key].pending =
        (countries[key].pending || 0) + (cm.openOrders || 0);
      countries[key].assigned =
        (countries[key].assigned || 0) + (cm.assignedOrders || 0);
      countries[key].outForDelivery =
        (countries[key].outForDelivery || 0) + (cm.outForDeliveryOrders || 0);
      countries[key].noResponse =
        (countries[key].noResponse || 0) + (cm.noResponseOrders || 0);
      countries[key].returned =
        (countries[key].returned || 0) + (cm.returnedOrders || 0);
      countries[key].cancelled =
        (countries[key].cancelled || 0) + (cm.cancelledOrders || 0);
      // amounts by status
      countries[key].amountTotalOrders =
        (countries[key].amountTotalOrders || 0) + (cm.amountTotalOrders || 0);
      const amtPending =
        (cm.amountOpen != null ? cm.amountOpen : cm.amountPending) || 0;
      countries[key].amountDelivered =
        (countries[key].amountDelivered || 0) + (cm.amountDelivered || 0);
      countries[key].amountPending =
        (countries[key].amountPending || 0) + amtPending;
      countries[key].amountDiscountDelivered =
        (countries[key].amountDiscountDelivered || 0) +
        (cm.amountDiscountDelivered || 0);
      countries[key].deliveredQty =
        (countries[key].deliveredQty || 0) + (cm.deliveredQty || 0);
    });
    ...
    for (const c of KNOWN_COUNTRIES) {
      ...
      const cur =
        countryCurrencyMap && countryCurrencyMap[c]
          ? countryCurrencyMap[c]
          : c === "KSA"
          ? "SAR"
          : c === "UAE"
          ? "AED"
          ...
          ? "QAR"
          : "AED";
      countries[c].deliveredQty = Number(
        deliveredQtyMap[c] || countries[c].deliveredQty || 0
      );
    ...
    console.log(
      `[Profit/Loss] Found ${deliveredOrders.length} delivered orders for profit calculation`,
      dateMatch.createdAt
        ? `(filtered by date: ${JSON.stringify(dateMatch.createdAt)})`
        : "(all time)"
    );

    // Log sample of order countries for debugging
    if (deliveredOrders.length > 0) {
      const sampleCountries = deliveredOrders
        .slice(0, 10)
        .map((o) => `"${o.orderCountry}"`)
        .join(", ");
    ...
    for (const inv of investors) {
      if (inv.investorProfile?.assignedProducts) {
        for (const ap of inv.investorProfile.assignedProducts) {
          ...
          investorCommissionMap.set(
            key,
            existing + Number(ap.profitPerUnit || 0)
          );
        }
      }
    }
    ...
    const canonicalizeCountry = (country) => {
      ...
      // Handle all variants
      if (upper === "KSA" || upper === "SAUDI ARABIA" || upper === "SA")
        return "KSA";
      if (upper === "UAE" || upper === "UNITED ARAB EMIRATES" || upper === "AE")
        return "UAE";
    ...
    console.log(
      `[Profit/Loss] Found ${adExpenses.length} advertisement expenses`,
      dateMatch.createdAt ? `(filtered by date)` : "(all time)"
    );
    ...
    console.log(
      "[Profit/Loss] Advertisement expenses by country:",
      Object.keys(adExpensesByCountry)
        .map((c) => `${c}: ${adExpensesByCountry[c].toFixed(2)}`)
        .join(", ")
    );
    ...
    for (const order of deliveredOrders) {
      ...
      if (revenue === 0) {
        console.warn(
          `[Profit/Loss] Order ${order._id} has zero total, country: ${orderCountry}`
        );
      }
      ...
      if (Array.isArray(order.items) && order.items.length > 0) {
        for (const item of order.items) {
          ...
          if (!prod) {
            console.warn(
              `[Profit/Loss] Order ${order._id} has item with no product reference`
            );
            continue;
          }
          if (!prod._id) {
            console.warn(
              `[Profit/Loss] Order ${order._id} has item with unpopulated product`
            );
            continue;
          }
          ...
          if (purchasePriceInBaseCurrency === 0) {
            console.warn(
              `[Profit/Loss] Product ${prod._id} has zero purchasePrice`
            );
          }

          // Convert purchase price from product's baseCurrency to order country currency
          const purchasePriceConverted = convertCurrency(
            purchasePriceInBaseCurrency,
            productBaseCurrency,
            countryCurrency
          );
        ...
        if (!prod || !prod._id) {
          console.warn(
            `[Profit/Loss] Order ${order._id} has unpopulated productId`
          );
        } else {
          ...
          if (purchasePriceInBaseCurrency === 0) {
            console.warn(
              `[Profit/Loss] Product ${prod._id} in order ${order._id} has zero purchasePrice`
            );
          }
        ...
        console.warn(
          `[Profit/Loss] Order ${order._id} has neither items nor productId`
        );
      }

      // Driver commission
      const driverCommission = order.deliveryBoy?.driverProfile
        ?.commissionPerOrder
        ? Number(order.deliveryBoy.driverProfile.commissionPerOrder)
        : 0;
      ...
      if (profitByCountry[canon]) {
        const countryCurrency = countryCurrencyMap[canon] || "AED";
        const agentCommissionLocal = convertCurrency(
          agentCommissionPKR,
          "PKR",
          countryCurrency
        );
        ...
        console.warn(
          `[Profit/Loss] Order country '${orderCountry}' (canonicalized to '${canon}') not in KNOWN_COUNTRIES. Order ID: ${order._id}`
        );
      }
    }
    ...
    console.log(
      "[Profit/Loss] Orders by country:",
      Object.keys(profitByCountry)
        .map(
          (c) =>
            `${c}: revenue=${profitByCountry[c].revenue.toFixed(
              2
            )}, purchaseCost=${profitByCountry[c].purchaseCost.toFixed(2)}`
        )
        .join(", ")
    );
    ...
    for (const c of KNOWN_COUNTRIES) {
      if (profitByCountry[c]) {
        profitByCountry[c].profit =
          profitByCountry[c].revenue -
          profitByCountry[c].purchaseCost -
          profitByCountry[c].driverCommission -
          profitByCountry[c].agentCommission -
          profitByCountry[c].investorCommission -
          profitByCountry[c].advertisementExpense;
      }
    }
    ...
    for (const c of KNOWN_COUNTRIES) {
      if (profitByCountry[c]) {
        ...
        totalInvestorCommAED += toAED(
          profitByCountry[c].investorCommission,
          curr
        );
        totalAdExpenseAED += toAED(
          profitByCountry[c].advertisementExpense,
          curr
        );
      }
